...
async function getAnalysisRuns(orgLogin: string, userId: string) {
  // 조직 확인 및 권한 체크
...
export default async function AnalysisListPage({
  ...
  const { runs, org } = await getAnalysisRuns(login, user.id);
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-muted-foreground">
            {org.name || org.login}의 모든 분석 실행 기록을 확인할 수 있습니다.
          </p>
        </div>
        <Button asChild>
          ...
          {runs.map((run) => {
            ...
            return (
              <Card key={run.id} className="hover:border-primary/50 transition-colors">
                <Link href={`/organizations/${login}/analysis/${run.id}`}>
